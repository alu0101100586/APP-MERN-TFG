  ...
  concert.save()
    .then((concertStorage) => {
      User.findOne({ _id: ownerId })
        ...
        .catch(() => {
          return res.status(500).send({ msg: "Error al actualizar el array de conciertos del usuario" });
        });
      ...
      Artist.findOne({ ownerId: ownerId })
        ...
        .catch(() => {
          return res.status(500).send({ msg: "Error al actualizar el array de conciertos del artista" });
        });
...
async function updateConcert(req, res) {
  ...
  if(Object.keys(concertData).length === 1 && concertData.hasOwnProperty('ownerId')) {
  ...
  Concert.findByIdAndUpdate({ _id: id, ownerId: ownerId }, concertData)
    .then((concertStorage) => {
      ...
      return res.status(200).send({ msg: "Concierto actualizado satisfactoriamente" });
    })
    .catch(() => {
  ...
  Concert.findByIdAndDelete({ _id: id, ownerId: ownerId })
    .then((concertStorage) => {
      ...
      User.findOne({ _id: ownerId })
        ...
        .catch(() => {
          return res.status(400).send({ msg: 'Error al actualizar el array de conciertos del usuario' });
        });
      ...
      Artist.findOne({ ownerId: ownerId })
        ...
        .catch(() => {
          return res.status(400).send({ msg: 'Error al actualizar el array de conciertos del artista' });
        });

      return res.status(200).send({ msg: 'Concierto eliminado satisfactoriamente' });
    })
    .catch(() => {
      ...
      Artist.findOne({ name: artistName })
        .then((artist) => {
          ...
          if (concertStorage.participants.includes(artist._id)) {
            return res.status(400).send({ msg: "El artista ya está en el concierto" });
          }
...
//Controlar que el artista que se mete existe en la base de datos, buscando por el nombre
//buacamos el nombre y metemos el id
  ...
  .then((concertStorage) => {
    ...
    Artist.findOne({ name: artistName })
      .then((artist) => {
        ...
        //Controlando que el artista no esté ya en el array de participantes
        if (!concertStorage.participants.includes(artist._id)) {
          return res.status(400).send({ msg: "El artista no está en el concierto" });
        }
