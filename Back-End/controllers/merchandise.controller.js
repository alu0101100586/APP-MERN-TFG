...
async function updateMerchandise(req, res) {
  ...
  if(Object.keys(merchData).length === 1 && merchData.hasOwnProperty('ownerId')) {
  ...
  Merchandise.findByIdAndUpdate({ _id: id, ownerId: ownerId }, merchData)
    .then((merchStorage) => {
      ...
      return res.status(200).send({ msg: 'Merchandise actualizado satisfactoriamente' });
    })
    .catch(() => {
  ...
  Merchandise.findByIdAndDelete({ _id: id, ownerId: ownerId })
    .then((merchStorage) => {
      ...
      User.findOne({ _id: ownerId })
        ...
        .catch(() => {
          return res.status(400).send({ msg: 'Error al actualizar el array de merchandise del usuario' });
        });
      ...
      Artist.findOne({ ownerId: ownerId })
        ...
        .catch(() => {
          return res.status(400).send({ msg: 'Error al actualizar el array de merchandise del artista' });
        });

      return res.status(200).send({ msg: 'Merchandise eliminado satisfactoriamente' });
    })
    .catch(() => {
