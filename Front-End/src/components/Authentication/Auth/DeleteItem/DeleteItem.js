...
import { initialValues, validationSchema } from './DeleteItem.form'
...
export function DeleteItem(props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formData) => {
      try {
        if (type === 'disc') {
          await discController.deleteDiscApi(accessToken, formData.item)
        } else if (type === 'concert') {
          await concertController.deleteConcertApi(accessToken, formData.item)
        } else if (type === 'merch') {
          await merchController.deleteMerchandiseApi(accessToken, formData.item)
        }
        onReload()
        close()
      } catch (error) {
