...
export function UpdateMerchandiseForm(props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formData) => {
      try {
        await merchService.updateMerchandiseApi(accessToken, formData, formData.item)
        onReload()
        close()
      } catch (error) {
        console.log(error);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const response = await merchService.getMerchandiseUserApi(accessToken, 1, 1000);
  ...
  const handleItemOptionChange = (event, { value }) => {
    ...
    const merchandise = merchandises.find(merchandise => merchandise._id === value);
    const merchReleaseDate = merchandise?.releaseDate ? new Date(merchandise.releaseDate).toISOString() : '';
