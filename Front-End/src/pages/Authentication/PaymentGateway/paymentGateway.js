...
import { Grid, Form, Input, Button, Icon, Modal, Header } from 'semantic-ui-react';
  ...
  useEffect(() => {
    (async () => {
      if (itemType === 'disc') {
        ...
        const merchandiseResponse = await merchandiseService.getMerchandiseApi(itemId);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formData) => {
      if(itemType === 'disc') {
        ...
        const response = await merchandiseService.buyMerchandiseApi(accessToken, itemId);
        setPaymentResponse(response);
      }
      setShowModal(true);
    },
