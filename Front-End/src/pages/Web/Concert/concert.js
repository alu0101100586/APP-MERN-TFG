...
export function Concert() {
  ...
  if (concert.musicalGenre.length >= 2) {
    ...
    genre =
      concert.musicalGenre
        .slice(0, concert.musicalGenre.length - 1)
        .join(', ') +
      ' y ' +
      lastGenre
  } else {
  ...
  if (concert.participants.length >= 2) {
    const lastParticipant =
      concert.participants[concert.participants.length - 1]
    participants =
      concert.participants
        .slice(0, concert.participants.length - 1)
        .join(', ') +
      ' y ' +
      lastParticipant
  } else {
      ...
      <Grid padded columns={3} stackable>
        ...
        <Grid.Column>
          <div className="concert-info">
            ...
            <p>
              ...
              <span style={{ fontWeight: 'bold' }}>{concert.price}€</span>. y se
              puede participar en el proyecto hasta el{' '}
              <span style={{ fontWeight: 'bold' }}>
                {formatDate(concert.date)}
              </span>
              . Tendrá una temática basada en{' '}
              <span style={{ fontWeight: 'bold' }}>{genre}</span>, y contará con
              la participación de{' '}
        ...
        <Grid.Column>
          <div className="crowdfunding-info">
            ...
            <p>
              Este proyecto de crowdfunding tiene como objetivo recaudar fondos
              para poder llevar a cabo el concierto de{' '}
              <span style={{ fontWeight: 'bold' }}>{artist.name}</span>. El
              dinero recaudado se destinará a pagar los gastos de la sala, el
              equipo de sonido y la publicidad del evento. Se necesita recaudar
              un mínimo de{' '}
              <span style={{ fontWeight: 'bold' }}>{concert.moneyLimit}€</span>{' '}
              para poder llevar a cabo el concierto, y se puede participar en el
              proyecto hasta el{' '}
              <span style={{ fontWeight: 'bold' }}>
                {formatDate(concert.date)}
              </span>
              . Se ha recaudado un total de{' '}
              <span style={{ fontWeight: 'bold' }}>{concert.raisedMoney}€</span>{' '}
              de y quedan un total de{' '}
              <span style={{ fontWeight: 'bold' }}>
                {concert.moneyLimit - concert.raisedMoney}€
              </span>
              para llegar al objetivo. El progreso de la campaña se puede ver en
              la siguiente barra de progreso:
            </p>
            <ProgressBar
              total={concert.moneyLimit}
              current={concert.raisedMoney}
            />
            {user &&
              user.role === 'common' &&
              new Date(concert.date) >= new Date() && (
                <Button
                  primary
                  onClick={() => navigate(`/auth/payment/concert-${concertId}`)}
                >
                  Apoyar la campaña
                </Button>
              )}
