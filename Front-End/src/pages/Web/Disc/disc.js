...
export function Disc() {
  ...
  if (disc.musicalGenre.length >= 2) {
    ...
    genre =
      disc.musicalGenre.slice(0, disc.musicalGenre.length - 1).join(', ') +
      ' y ' +
      lastGenre
  } else {
      ...
      <Grid padded columns={3} stackable>
        ...
        <Grid.Column>
          <div className="disc-info">
            ...
            <p>
              ...
              <span style={{ fontWeight: 'bold' }}>{artist.name}</span>. Tiene
              un precio de{' '}
              <span style={{ fontWeight: 'bold' }}>{disc.price}€</span> y se
              puede participar en la campaña hasta{' '}
              <span style={{ fontWeight: 'bold' }}>
                {formatDate(disc.releaseDate)}
              </span>
              . El disco está pensado para los amantes de la música{' '}
              <span style={{ fontWeight: 'bold' }}>{genre}</span>. Tendrá
              disponibles las siguientes canciones:
            </p>
            <p>
        ...
        <Grid.Column>
          <div className="crowdfunding-info">
            ...
            <p>
              Para poder llevar a cabo este proyecto, el artista necesita
              recaudar{' '}
              <span style={{ fontWeight: 'bold' }}>{disc.moneyLimit}€</span>{' '}
              antes de{' '}
              <span style={{ fontWeight: 'bold' }}>
                {formatDate(disc.releaseDate)}
              </span>
              ...
              <span style={{ fontWeight: 'bold' }}>
                {disc.moneyLimit - disc.raisedMoney}€
              </span>{' '}
              por recaudar. El progreso de la campaña se puede ver en la
              siguiente barra de progreso:
            </p>
            <ProgressBar total={disc.moneyLimit} current={disc.raisedMoney} />
            {user &&
              user.role === 'common' &&
              new Date(disc.releaseDate) >= new Date() && (
                <Button
                  primary
                  onClick={() => navigate(`/auth/payment/disc-${discId}`)}
                >
                  Apoyar la campaña
                </Button>
              )}
